...
export function TodayPage({ firstName }: { firstName: string }) {
  ...
  // Accounts with meetings (recently active)
  const activeAccounts = accounts.filter(a => a.meetings && a.meetings.length > 0)

  // Accounts with pre-enriched contacts but no meetings yet
  const accountsToMeet = accounts.filter(a => {
    const hasContacts = a.contacts?.length || PRE_ENRICHED[a.company]?.length
    const noMeetings = !a.meetings?.length
    return hasContacts && noMeetings && a.priority === 'High'
  }).slice(0, 5)
  ...
  return (
    <div className="max-w-[800px] mx-auto px-6 py-8">
      {/* Greeting */}
      <div className="mb-8">
        <h1 className="text-[28px] font-bold text-text">Good morning, {firstName}</h1>
      ...
      {/* Follow-ups Due */}
      {pendingFollowUps.length > 0 && (
        <Section title={`Follow-ups to Send (${pendingFollowUps.length})`} icon="📧">
      ...
      {upcomingEvents.length > 0 && (
        <Section title={`Events You're Attending (${upcomingEvents.length})`} icon="📅">
          {upcomingEvents.map(evt => (
            <EventRow key={evt.id} event={evt} accounts={accounts} />
          ))}
        </Section>
      )}
      ...
      {accountsToMeet.length > 0 && (
        <Section title="High-Priority Accounts to Meet" icon="🎯">
          {accountsToMeet.map(a => (
            <AccountToMeetRow key={a.id} account={a} events={events} onLogMeeting={(meeting) => {
              const meetings = [...(a.meetings || []), meeting]
              const stage: Stage = 'Met'
              updateAccount(a.id, { meetings, stage })
              setTimeout(save, 100)
            }} />
          ))}
        </Section>
      ...
      {notAttendingEvents.length > 0 && (
        <Section title="Events to Consider" icon="🗓️">
          {notAttendingEvents.map(evt => (
            <div key={evt.id} className="flex items-center justify-between py-3 border-b border-border last:border-b-0">
              <div>
              ...
              <button
                ...
                className="text-[12px] font-medium text-blue hover:text-blue2 cursor-pointer"
              >
                Mark Attending
              </button>
            </div>
      ...
      {/* Gmail status */}
      {!gmailConnected && (
        <div className="mt-6 p-4 bg-amber-bg border border-amber-border rounded-xl text-[13px] text-amber">
...
function FollowUpCard({ item }: { item: { account: Account; meeting: Meeting; meetingIdx: number; fuIdx: number; fu: Meeting['followUps'][0] } }) {
  ...
  if (sent) {
    return (
      <div className="p-4 bg-green-bg border border-green-border rounded-xl mb-3 text-[13px] text-green font-medium">
  ...
  return (
    <div className="p-4 bg-white border border-border rounded-xl mb-3 shadow-sm">
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="text-[14px] font-semibold text-text">{item.meeting.contact}</div>
          <div className="text-[12px] text-text2">{item.account.company} · Day {item.fu.day} follow-up</div>
          <div className="text-[11px] text-text3 mt-0.5">Met at {item.meeting.event} · {item.meeting.date}</div>
        </div>
        {item.meeting.contactEmail ? (
          <span className="text-[11px] text-green font-medium bg-green-bg px-2 py-0.5 rounded-full">{item.meeting.contactEmail}</span>
        ) : (
          <span className="text-[11px] text-amber font-medium bg-amber-bg px-2 py-0.5 rounded-full">No email</span>
        )}
      </div>

      {/* Email preview */}
      <div className="bg-surface2 rounded-lg p-3 mb-3">
        <div className="text-[12px] font-semibold text-text mb-1">Subject: {item.fu.subject}</div>
        <div className="text-[13px] text-text2 leading-relaxed whitespace-pre-wrap">{item.fu.body}</div>
      </div>

      {/* Meeting notes context */}
      <div className="text-[12px] text-text3 italic mb-3">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="px-4 py-2 text-[13px] font-semibold bg-blue text-white rounded-lg hover:bg-blue2 cursor-pointer disabled:opacity-40 transition-colors"
        >
          {sending ? 'Sending...' : 'Approve & Send'}
        </button>
        <button className="px-4 py-2 text-[13px] font-medium border border-border rounded-lg text-text2 hover:bg-surface2 cursor-pointer transition-colors">
          Edit
        </button>
        <button className="px-4 py-2 text-[13px] font-medium border border-border rounded-lg text-text3 hover:bg-surface2 cursor-pointer transition-colors">
          Skip
        </button>
      </div>
    </div>
...
/* ── Event Row ── */
function EventRow({ event, accounts }: { event: BdrEvent; accounts: Account[] }) {
  ...
  return (
    <div className="py-3 border-b border-border last:border-b-0">
      <div className="flex items-start justify-between">
        ...
        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${event.relevance === 'High' ? 'bg-amber-bg text-amber' : 'bg-surface2 text-text3'}`}>
          {event.relevance}
        </span>
      </div>
      {nearby.length > 0 && (
...
/* ── Account to Meet Row ── */
function AccountToMeetRow({ account, events, onLogMeeting }: { account: Account; events: BdrEvent[]; onLogMeeting: (m: Meeting) => void }) {
    ...
    <div className="flex items-center justify-between py-3 border-b border-border last:border-b-0">
      <div>
        ...
        <div className="text-[12px] text-text3 mt-0.5">
          {account.city} · {account.vertical} · {contacts.length} contacts
        </div>
      </div>
      <button
        onClick={handleLogMeeting}
        disabled={logging}
        className="text-[12px] font-semibold text-white bg-blue px-3 py-1.5 rounded-lg hover:bg-blue2 cursor-pointer disabled:opacity-40 transition-colors"
      >
...
function Section({ title, icon, children }: { title: string; icon: string; children: React.ReactNode }) {
  return (
    <div className="mb-8">
      <h2 className="text-[16px] font-semibold text-text mb-3 flex items-center gap-2">
